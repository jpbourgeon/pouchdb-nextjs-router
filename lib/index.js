...
// Build the router once: order matters.
const router = []
  .concat(
    baseRoutes,
    changesRoutes,
  ...
  .map((route) => ({
    ...route,
    regexp: pathToRegexp(route.pathToRegexp).regexp,
  }));

const bodyParsers = new Map();

const getBodyParsers = (limit) => {
  const configuredLimit = limit || "1mb";
  let parsers = bodyParsers.get(configuredLimit);

  if (!parsers) {
    parsers = {
      json: json({ limit: configuredLimit }),
      raw: raw({ limit: configuredLimit, type: "*/*" }),
    };
    bodyParsers.set(configuredLimit, parsers);
  }
    ...
    for (let i = 0; i < router.length; i++) {
      ...
      if (
        ...
        if (route.parseJsonBody) {
          ...
          await runMiddleware(req, res, getBodyParsers(limit).json);
        }
        ...
        if (
          ...
          await runMiddleware(req, res, getBodyParsers(limit).raw);
        }
        req.locals.nextPouchDBRouter.body = req.body;

        // run the onRequest middlewares
        if (req.locals.nextPouchDBRouter.middleware?.onRequest?.length)
        ...
        if (req.locals.nextPouchDBRouter.middleware?.onResponse?.length)
