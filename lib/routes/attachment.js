...
attachmentRoutes.push({
  name: "Put design doc attachment",
  method: "PUT",
  ...
  handler: async (req, res) => {
    await putAttachment(
      req,
      res,
      "_design/" + req.query.params?.[1],
      req.query.params?.splice(3).join("/")
    );
  },
});
  ...
  handler: async (req, res) => {
    ...
    if (
      ...
      await putAttachment(
        ...
        res,
        req.query.params?.[1],
        req.query.params?.splice(2).join("/")
      );
    }
  },
});
...
attachmentRoutes.push({
  name: "Get design doc attachment",
  method: "GET",
  pathToRegexp: "/:db/_design/:id/:attachment(.*)",
  handler: async (req, res) => {
    await getAttachment(
...
attachmentRoutes.push({
  name: "Delete design doc attachment",
  method: "DELETE",
  pathToRegexp: "/:db/_design/:id/:attachment(.*)",
  handler: async (req, res) => {
